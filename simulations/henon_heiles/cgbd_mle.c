...
int main(int argc, char *argv[]) {
    double total_time = get_double(argc, argv, 1);
    size_t grid_size = get_size_t(argc, argv, 2);
    size_t n_threads = get_size_t(argc, argv, 3);
    ...
    char filename[512];
    snprintf(filename, sizeof(filename), "%s/cgbd_mle_T=%.1f_grid_size=%zu.dat",
             path, total_time, grid_size);
...
#pragma omp parallel for
    for (size_t g = 0; g < grid_size * grid_size; g++) {
        ...
        double E = E_range[0] + j * (E_range[1] - E_range[0]) / (grid_size - 1);
